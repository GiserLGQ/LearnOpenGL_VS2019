...
int main()
{
    ...
    // 告诉 stb_image.h 在 y 轴上翻转加载的纹理（在加载模型之前）。
    ...
    Shader modelShader("ShaderPath/main_model_lighting/model.vs", "ShaderPath/main_model_lighting/model.fs");
    Shader lightshader("ShaderPath/main_model_lighting/light.vs", "ShaderPath/main_model_lighting/light.fs");


    // 加载模型
    Model ourModel((char*)"../resourse/nanosuit/nanosuit.obj");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        */
        ...
        lightPos.x = cos(glfwGetTime()) * 5.0f;
        lightPos.z = sin(glfwGetTime()) * 5.0f;
        ...
        modelShader.setVec3("pointLights.position", lightPos);

        modelShader.setVec3("pointLights.ambient", 0.8f, 0.8f, 0.8f);
        modelShader.setVec3("pointLights.diffuse", 1.f, 1.f, 1.f);
       ...
       // model = glm::rotate(model, (float)glfwGetTime(), glm::vec3(0.f, 1.0f, 0.f));
        ...
        //   //首先创建一个模型矩阵
        model = glm::mat4(1.0f);
        model = glm::translate(model, lightPos) * glm::scale(model, glm::vec3(0.5f));
